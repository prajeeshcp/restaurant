<?php $this->load->view('includes/header'); ?>
		...
		<?php $this->load->view('includes/sidebar', array('selected' => 'tabledetails')); ?>
		<!-- END NAVIGATION -->
		...
		<article class="col-sm-12 col-md-12 col-lg-6">
			...
			<div class="jarviswidget" id="wid-id-2" data-widget-colorbutton="false"	data-widget-editbutton="false">
				...
				<header>
					...
					<h2><?=(empty($get_data)) ? 'Add New Category ' : 'Edit '.$get_data['table_number']?></h2>				
				...
				<div>
					...
					<div class="widget-body no-padding">
						...
						<form action="<?=site_url('manage/submit_table_details')?>" method="post" class="smart-form">


                        <?php if (!empty($get_data)) { ?>
                        <input type="hidden" name="edit_id" value="<?=$get_data['id']?>" />
                        <?php } ?>
							<fieldset>					
								<section>
									<label class="label">Table Number </label>
									<label class="input state-success">
										<input type="text" value="<?=(!empty($get_data)) ? $get_data['table_number'] : '' ?>" name="table_number" required>
									</label>
								...
								<section>
									...
									<label class="input state-success">
										<input type="text" value="<?=(!empty($get_data)) ? $get_data['capacity'] : '' ?>" name="capacity" required>
									</label>
									<div class="note note-success">This is a required field.</div>
								</section>
                                ...
                                <section>
									...
									<label class="select state-success">
										<select id="" name="table_category" required>
                                         <option value="">SELECT CATEGORY</option>
                                        <?php if (!empty($category)) {
												foreach ($category as $cat) {
											 ?>
											<option <?=(!empty($get_data) && $get_data['table_cat_id'] == $cat['id'])  ? 'selected' : ''?> value="<?=$cat['id']?>"><?=stripslashes($cat['name'])?></option>
											<?php } } ?>
										</select>
										<i></i>
									</label>
								...
								<section>
									...
									<label class="select state-success">
										<select id="" name="table_status" required>
